...
export default class TransitionItemsView extends React.Component<
  ...
  getIsPartOfSharedTransition = (name: string, route: string) => {
    ...
    const { fromRoute, toRoute } = this.state;
    let sharedElements = this._transitionItems.getSharedElements(fromRoute, toRoute);
    sharedElements=this._filterZeroElements(sharedElements);
  ...
  _filterZeroElements=elements=>elements.filter(el=>!(
    el.fromItem.metrics.width===0
    && el.fromItem.metrics.height===0
    || (
      el.toItem.metrics.width===0
      && el.toItem.metrics.height===0
    ))
  );
